...
export default function AdminBookingsPage() {
  ...
  const getStatusBadge = (status: BookingStatus) => {
    switch (status) {
      case "pending":
        return <Badge className="bg-status-pending/20 text-status-pending border-0 text-xs">{t('booking.pending')}</Badge>
      case "approved":
        return <Badge className="bg-status-approved/20 text-status-approved border-0 text-xs">{t('booking.approved')}</Badge>
      case "rejected":
        return <Badge className="bg-status-rejected/20 text-status-rejected border-0 text-xs">{t('booking.rejected')}</Badge>
    }
  }
          ...
          <Card className="bg-card border-border">
            <CardHeader className="p-4 md:p-6 pb-4">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
                <div>
                  <CardTitle className="text-base md:text-lg">{t('booking.bookingRequests')}</CardTitle>
              ...
              {isLoading ? (
                ...
                <>
                        ...
                        <CardContent className="p-4">
                          <div className="flex items-start justify-between gap-3 mb-3">
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                              ...
                              <div className="min-w-0 flex-1">
                                <p className="font-medium text-sm truncate">{booking.user?.name || 'Unknown User'}</p>
                                <p className="text-xs text-muted-foreground truncate">{booking.user?.email || 'N/A'}</p>
                              </div>
                            </div>
                          ...
                          <div className="space-y-2 mb-3">
                            <div className="flex items-center gap-2">
                              <p className="text-xs text-muted-foreground">{t('booking.space')}:</p>
                              <p className="text-sm font-medium">{booking.space?.name || 'Unknown Space'}</p>
                              <Badge variant="outline" className="text-xs capitalize ml-auto">
                                {booking.space?.type || 'N/A'}
                              </Badge>
                            </div>
                            <div className="flex items-center gap-2">
                          ...
                          {booking.status === "pending" && (
                            <div className="flex flex-col gap-2 pt-3 border-t border-border">
                              <Button
                                size="sm"
                                variant="outline"
                                className="gap-1 border-status-approved text-status-approved hover:bg-status-approved hover:text-background bg-transparent w-full"
                              ...
                              <Button
                                size="sm"
                                variant="outline"
                                className="gap-1 border-status-rejected text-status-rejected hover:bg-status-rejected hover:text-foreground bg-transparent w-full"
                          ...
                          >
                            <td className="py-3 md:py-4 px-3 md:px-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <p className="font-medium text-sm md:text-base">{booking.user?.name || 'Unknown User'}</p>
                                  <p className="text-xs md:text-sm text-muted-foreground">{booking.user?.email || 'N/A'}</p>
                                </div>
                              </div>
                            </td>
                            <td className="py-3 md:py-4 px-3 md:px-4">
                              <p className="font-medium text-sm md:text-base">{booking.space?.name || 'Unknown Space'}</p>
                            ...
                            <td className="py-3 md:py-4 px-3 md:px-4">{getStatusBadge(booking.status)}</td>
                            <td className="py-3 md:py-4 px-3 md:px-4">
                              {booking.status === "pending" && (
                                <div className="flex items-center gap-2 flex-wrap">
                                  ...
                                  >
                                    ...
                                    <span className="hidden lg:inline">{t('booking.approve')}</span>
                                  </Button>
                                  <Button
                                    size="sm"
                                    variant="outline"
                                  ...
                                  >
                                    ...
                                    <span className="hidden lg:inline">{t('booking.reject')}</span>
                                  </Button>
                                </div>
                              )}
                            </td>
                  ...
                  {filteredBookings.length > 0 && totalPages > 1 && (
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-2 md:gap-4 mt-4 md:mt-6 pt-4 border-t border-border">
                  ...
                  {filteredBookings.length > 0 && totalPages <= 1 && (
                    <div className="mt-4 md:mt-6 pt-4 border-t border-border text-center text-xs md:text-sm text-muted-foreground">
